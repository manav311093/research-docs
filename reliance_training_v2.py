...
from kiteconnect.util import get_daily_data
...
def preprocess_data(
    ...
    # volatility features
    candles_df = candles_df.with_columns(
        volatility_percentile=pl.col("price_volatility").cumulative_eval(
            (pl.element().rank(method="average") / pl.element().len()).tail(1)
        ),
        volatility_expanding=(pl.col("volatility_std_over_3m") > pl.col('volatility_std_over_15m')).cast(pl.Int8),
        atr_percentile=pl.col("atr_14").cumulative_eval(
            (pl.element().rank(method="average") / pl.element().len()).tail(1)
        ),
    ...
    candles_df = join_daily_features(DAILY_DF.clone(), candles_df)
    candles_df = candles_df.with_columns(
        volatility_norm_daily=pl.col('price_volatility') / pl.col('d_ret_std_20'),
        atr_norm_daily=pl.col("atr_14") / pl.col('d_range_mean_20')
    )

    return candles_df


def build_daily_features(daily_df: pl.DataFrame) -> pl.DataFrame:
    # Here you can add any daily-level features if needed
    daily_df = daily_df.with_columns(
        prev_close=pl.col('close').shift(1),
        daily_return=((pl.col('close') - pl.col('open')) / pl.col('close')).fill_null(0),
        daily_range=((pl.col('high') - pl.col('low')) / pl.col('close')).fill_null(0),
        gap_pct=((pl.col('open') - pl.col('close').shift(1)) / pl.col('close').shift(1)).fill_null(0),
        volume_price=(pl.col('volume') * pl.col('close')).cast(pl.Float64).fill_null(0),
    )
    # rolling stats then shft 1 to avoid lookahead bias
    daily_df = daily_df.with_columns(
        d_ret_std_5=pl.col('daily_return').rolling_std(window_size=5, min_samples=3).shift(1),
        d_ret_std_10=pl.col('daily_return').rolling_std(window_size=10, min_samples=5).shift(1),
        d_ret_std_20=pl.col('daily_return').rolling_std(window_size=20, min_samples=10).shift(1),
        d_range_mean_5=pl.col('daily_range').rolling_mean(window_size=5, min_samples=3).shift(1),
        d_range_mean_10=pl.col('daily_range').rolling_mean(window_size=10, min_samples=5).shift(1),
        d_range_mean_20=pl.col('daily_range').rolling_mean(window_size=20, min_samples=10).shift(1),
        d_gap_abs_mean_5=pl.col('gap_pct').abs().rolling_mean(window_size=5, min_samples=3).shift(1),
        d_gap_abs_mean_10=pl.col('gap_pct').abs().rolling_mean(window_size=10, min_samples=5).shift(1),
        d_gap_abs_mean_20=pl.col('gap_pct').abs().rolling_mean(window_size=20, min_samples=10).shift(1),
        d_prev_day_ret=pl.col('daily_return').shift(1),
        d_prev_day_range=pl.col('daily_range').shift(1),
        d_volume_price_mean_5=pl.col('volume_price').rolling_mean(window_size=5, min_samples=3).shift(1),
        d_volume_price_mean_10=pl.col('volume_price').rolling_mean(window_size=10, min_samples=5).shift(1),
        d_volume_price_mean_20=pl.col('volume_price').rolling_mean(window_size=20, min_samples=10).shift(1),
    )
    # daily volitility features derived from rolling vol vs its own ewm mean/std
    daily_df = daily_df.with_columns(
        d_volatility_ewm_mean=pl.col('d_ret_std_20').ewm_mean(span=60, adjust=False, min_samples=10),
        d_volatility_ewm_std=pl.col('d_ret_std_20').ewm_std(span=60, adjust=False, min_samples=10),
    ).with_columns(
        d_volatility_zscore=((pl.col('d_ret_std_20') - pl.col('d_volatility_ewm_mean')) / (pl.col('d_volatility_ewm_std') + 1e-9)).fill_null(0),
    ).with_columns(
        d_vol_regime=pl.when(pl.col('d_volatility_zscore') < -0.5).then(0)
          .when(pl.col('d_volatility_zscore') >0.5).then(2)
          .otherwise(1).cast(pl.Int8)
    ).drop(['d_volatility_ewm_mean', 'd_volatility_ewm_std', 'd_volatility_zscore'])
    return daily_df

def join_daily_features(daily_df: pl.DataFrame, minute_df: pl.DataFrame) -> pl.DataFrame:
    # Join daily features with candle features
    minute_df = minute_df.with_columns(
        trade_date=pl.col("timestamp").dt.date()
    )
    daily_df = daily_df.with_columns(
        trade_date=pl.col("timestamp").dt.date()
    ).drop('timestamp')
    
    return minute_df.join(daily_df, on="trade_date", how="left")
...
def generate_good_trade_label(df: pl.DataFrame,
    ...
    def calculate_label_for_single_tick(
        ...
        threshold_time = current_timestamp + datetime.timedelta(minutes=60)
        ...
        for j in range(current_idx + 1, len(group_prices)):
            ...
            if future_path_timestamp > threshold_time:
                break  # Stop evaluating further if beyond threshold time
...
DAILY_DF = build_daily_features(pl.DataFrame(get_daily_data(TOKEN, "2025-04-01", "2026-01-10")))
# --- Main Execution ---
if __name__ == "__main__":
    ...
    if TRAIN_MODEL:
        for day in TRAIN_DAYS:
            ...
            output_df = output_df.filter(pl.col("tradeable").is_not_null())
        ...
        feature_columns = ['volatility_std_over_15m', 'volatility_std_over_3m',
                            'volatility_mean_over_15m', 'volatility_mean_over_3m',
                            'volatility_ratio',
                            ...
                            'obi_diff_315', 'obi_diff_315_abs', 'obi_diff_315_slope',
                            'obi_diff_35', 'obi_diff_35_abs', 'obi_diff_35_slope', 'session_segment',

                            ## new
                            'd_ret_std_5', 'd_gap_abs_mean_5', 'd_vol_regime',
                            'd_ret_std_10', 'd_gap_abs_mean_10',
    ...
    for day in PNL_DAYS:
        ...
        if SHOW_PLOT:
            ...
            buy_signals = df[(df['buy_signal'].shift(1) == True) & (df['tradeable'].shift(1) == 1)]
            ...
            sell_signals = df[(df['sell_signal'].shift(1) == True) & (df['tradeable'].shift(1) == 1)]
